...
export default function CarDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <main className="min-h-[90vh] pt-28 pb-16 px-2 bg-white flex flex-col items-center">
      <div className="w-full bg-white border border-gray-200 rounded-3xl shadow-lg p-8 flex flex-col md:flex-row gap-8 items-center md:items-start">
        <img
          src={car.image}
          alt={`${car.make} ${car.model}`}
          className="w-full md:w-1/2 h-64 object-cover rounded-2xl border border-gray-100 shadow mb-6 md:mb-0"
        />
        <div className="flex-1 flex flex-col gap-4 w-full">
          ...
          <button className="px-8 py-3 bg-blue-600 text-white text-lg font-bold rounded-full shadow hover:bg-blue-700 transition-all w-full md:w-auto">
            Start Purchase
          </button>
